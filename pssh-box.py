...
# changed to output only the pssh data.
...
class Pssh(object):
  ...
  def __init__(self, version, system_id, key_ids, pssh_data, include_pssh_atom):
    ...
    self.include_pssh_atom = include_pssh_atom

  def binary_string(self):
    """Converts the PSSH box to a binary string."""
    if self.include_pssh_atom == "true":
...
def _parse_boxes(data, include_pssh_atom):
  ...
  while reader.has_data():
    ...
    pssh = Pssh(version, system_id, key_ids, pssh_data, include_pssh_atom)
    boxes.append(pssh)
  return boxes
...
reasons.""")
  ...
  extra.add_argument('--include-pssh-atom',
                     choices=['true', 'false'],
                     help='Indicates if the PSSH box must contain the pssh atom')
...
def main(all_args):
  ...
  for args in arg_groups:
    ...
    if ns.input:
      boxes.extend(_parse_boxes(ns.input, ns.include_pssh_atom))
    ...
    boxes.append(Pssh(version, ns.system_id, ns.key_id, pssh_data, ns.include_pssh_atom))
